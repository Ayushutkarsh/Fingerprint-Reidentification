...
class Dataset:
    def __init__(self, train_color_list, train_thermal_list):
        ...
        #self.n_classes = 206
        self.n_classes = 16 # only 16 classes in my test
    
    def read_img(self, data_path):
        #img = cv2.imread('../Dataset/' + data_path)
        img = cv2.imread(data_path) #in my test whole path is used
        h, w, c = img.shape
        assert c==3
    ...
    def next_batch(self, batch_size, phase):
        ...
        if phase == 'train':
            ...
            a = np.random.permutation(self.n_classes+1)
            #print("&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&",a)
            b = np.array([0])
            idx = np.setdiff1d(a,b)
            batch_idx = idx[:batch_size]
            for i in xrange(batch_size):
                # random select one color image and one thermal image for each identity 
                tmp_label = batch_idx[i]
                #print("############## my train labels######",self.train_color_label,"$$$$$$$$$" , tmp_label,"$$$$$$$$$$$")
