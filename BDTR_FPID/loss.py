...
def gather_2d(params, indices):
    ...
    print("#######################",shape)
    if shape[1] is not None and shape[1]>0:
...
def compute_birank_loss(feat1, feat2, batch_size, margin = 0.5, lambda_intra = 0.1):
    ...
    idx1 = tf.concat(axis=1, values=[tf.reshape(pos_positive,[batch_size,1]),tf.reshape(pos_negative,[batch_size,1])])
    ...
    idx2 = tf.concat(axis=1, values=[tf.reshape(pos_positive,[batch_size,1]),tf.reshape(pos_negative,[batch_size,1])])
