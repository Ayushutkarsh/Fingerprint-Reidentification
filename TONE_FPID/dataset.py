...
class Dataset:
    def __init__(self, train_color_list, train_thermal_list):
        ...
        with open(train_color_list) as f:
            ...
            self.train_color_label = [int(s.split(' ')[1])-1 for s in data_color_list]
        ...
        with open(train_thermal_list) as f:
            ...
            self.train_thermal_label = [int(s.split(' ')[1])-1 for s in data_thermal_list]
    ...
    def next_batch(self, batch_size, phase):
        ...
        #print("&&&&&&&&&&&&&&&&" , batch_color_labels.shape)
        ...
        for i in xrange(len(thermal_paths)):
            # 
            img = cv2.imread(thermal_paths[i])
            h, w, c = img.shape
            assert c==3
